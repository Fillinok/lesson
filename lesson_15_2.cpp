...
public:
    ...
    [[nodiscard]] bool AddDocument(int document_id, const string& document, DocumentStatus status, const vector<int>& ratings) {
        ...
        document_id_in_series_.push_back(document_id);
        return true;
    ...
    [[nodiscard]] bool FindTopDocuments(const string& raw_query, DocumentPredicate document_predicate,vector<Document>& result) const {       
        ...
        for(const auto q:query.plus_words)
        {
            if(!IsValidWord(q))
            {
                return false;
        ...
        for(const auto q:query.minus_words)
        {
            if(!IsValidWord(q) || q[0] == '-' || q.empty())
            {
                return false;
            }
        }
    ...
    int GetDocumentId(int index) const {        
        if(index >= 0 && index < document_id_in_series_.size())
        {           
            return document_id_in_series_[index];
        }
        else 
        {            
...
private:
    ...
    vector<int> document_id_in_series_;
...
int main() {
    ...
    (void) search_server.AddDocument(-2, "пушистый кот пушистый хвост"s, DocumentStatus::ACTUAL, {7, 2, 7});    
    (void)search_server.AddDocument(0, "кот в мешке", DocumentStatus::ACTUAL, {1, 2, 3});
    (void)search_server.AddDocument(1, "чук и гек пьют иван-чай", DocumentStatus::ACTUAL, {3});
    if (vector<Document> docs; !search_server.FindTopDocuments("в", docs) || !docs.empty()) { 
        cout << "поиск стоп-слова должен завершаться успешно и возвращать пустой результат" << endl;
}
    ...
    if (search_server.FindTopDocuments("чук"s, documents)) {
